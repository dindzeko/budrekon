...
def perform_vouching(rk_df, sp2d_df):
    ...
    skpd_cols = [col for col in sp2d_df.columns if any(keyword in col for keyword in ['skpd', 'unit', 'bagian'])]
    if not skpd_cols:
    ...
    # Deduplikasi
    sp2d_df = sp2d_df.drop_duplicates(subset=['nosp2d_6digits', 'jumlah'], keep='first')
    rk_df = rk_df.drop_duplicates(subset=['nosp2d_6digits', 'jumlah'], keep='first')
