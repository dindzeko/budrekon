...
# Fungsi untuk mengekstrak nomor SP2D dari keterangan
def extract_sp2d_number(description):
    """
    Fungsi untuk mengekstrak 6 digit nomor SP2D dari keterangan.
    Mendukung berbagai jenis SP2D seperti LS, UP, GU, TU.
    """
    match = re.search(r'(?i)(?:ls|up|gu|tu)/\d{6}', str(description))
    if match:
        return match.group(0)[-6:]  # Ambil hanya 6 digit terakhir
    return None

# Fungsi untuk menormalisasi nama SKPD
def normalize_skpd(skpd_name):
    """Fungsi untuk menormalisasi nama SKPD"""
    skpd_mapping = {
        ...
        "disdik": "disdik",
        # Tambahkan mapping lain jika ada kasus serupa
    }
    skpd_name = str(skpd_name).lower().strip()
    return skpd_mapping.get(skpd_name, skpd_name)

# Fungsi utama untuk vouching
@st.cache_data
def perform_vouching(rk_df, sp2d_df):
    ...
    # Normalisasi SKPD
    rk_df['skpd_normalized'] = rk_df['keterangan'].apply(normalize_skpd)
    sp2d_df['skpd_normalized'] = sp2d_df['skpd'].apply(normalize_skpd)

    # Membuat kunci pencocokan
    rk_df['key'] = rk_df['nosp2d_6digits'] + '_' + rk_df['jumlah'].astype(str) + '_' + rk_df['skpd_normalized']
    sp2d_df['key'] = sp2d_df['nosp2d_6digits'] + '_' + sp2d_df['jumlah'].astype(str) + '_' + sp2d_df['skpd_normalized']
    ...
    if not unmatched_rk.empty and not unmatched_sp2d.empty:
        second_merge = unmatched_rk.merge(
            unmatched_sp2d,
            left_on=['jumlah', 'tanggal', 'skpd_normalized'],
            right_on=['jumlah', 'tglsp2d', 'skpd_normalized'],
            how='inner',
            suffixes=('', '_y')
        )
