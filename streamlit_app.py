...
# Fungsi untuk ekstraksi nomor SP2D
def extract_sp2d_number(description):
    match = re.search(r'(?<!\d)\d{6}(?!\d)', str(description))
    return match.group(0) if match else None

# Fungsi untuk mengekstrak SKPD dari uraian
def extract_skpd(description, skpd_mapping):
    for key in skpd_mapping:
        if key.lower() in str(description).lower():
            return skpd_mapping[key]
    return None
...
def perform_vouching(rk_df, sp2d_df):
    ...
    # Layer baru: Normalisasi SKPD untuk data yang masih unmatched
    if not unmatched_rk.empty and not unmatched_sp2d.empty:
        # Mapping SKPD
        skpd_mapping = {
        ...
        # Ekstraksi SKPD di RK
        unmatched_rk['skpd'] = unmatched_rk['keterangan'].apply(lambda x: extract_skpd(x, skpd_mapping))

        # Normalisasi SKPD di SP2D
        unmatched_sp2d['skpd'] = unmatched_sp2d['skpd'].apply(
            lambda x: next((v for k, v in skpd_mapping.items() if k.lower() in str(x).lower()), x)
        )

        # Matching berdasarkan jumlah, tanggal, dan SKPD
        ...
        if not third_merge.empty:
            ...
            merged.loc[third_merge.index, 'status'] = 'Matched (Tertiary)'
...
if rk_file and sp2d_file:
    try:
        ...
        cols[3].metric("Matched (Tertiary)", len(merged_rk[merged_rk['status'] == 'Matched (Tertiary)']))
