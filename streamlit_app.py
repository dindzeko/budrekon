...
def extract_sp2d_number(description):
    """Ekstrak 6 digit pertama No.SP2D dari keterangan RK"""
    match = re.search(r'/(\d{6})/', str(description))  # Cari pola /000292/
    return match.group(1) if match else None

def extract_skpd_keterangan(description):
    """Ekstrak kode SKPD dari keterangan RK (contoh: LARANGAN)"""
    parts = str(description).split('/')
    if len(parts) >= 4:
        return parts[3].strip()  # Ambil bagian ke-4 (indeks 3)
    return None

def extract_skpd_code(skpd_name):
    """Ekstrak kode SKPD dari nama lengkap SP2D (contoh: 44 Kecamatan Larangan → LARANGAN)"""
    if isinstance(skpd_name, str) and 'Kecamatan' in skpd_name:
        return skpd_name.split('Kecamatan ')[-1].strip()
    return skpd_name.strip()
...
def perform_vouching(rk_df, sp2d_df):
    ...
    sp2d_df['skpd_code'] = sp2d_df['skpd'].apply(extract_skpd_code)  # Kode SKPD dari nama
    
    # Konversi tanggal
    rk_df['tanggal'] = pd.to_datetime(rk_df['tanggal'], errors='coerce')
    sp2d_df['tglsp2d'] = pd.to_datetime(sp2d_df['tglsp2d'], errors='coerce')
    
    # Vouching pertama: Kunci utama (No.SP2D + Jumlah)
    merged = rk_df.merge(
        sp2d_df[['nosp2d_6digits', 'jumlah', 'nosp2d', 'tglsp2d', 'skpd', 'skpd_code']],
    ...
    # Identifikasi data yang belum terhubung
    used_sp2d = set(merged.loc[merged['status'] == 'Matched', 'nosp2d'])
    unmatched_sp2d = sp2d_df[~sp2d_df['nosp2d'].isin(used_sp2d)]
    ...
    if not unmatched_rk.empty and not unmatched_sp2d.empty:
        second_merge = unmatched_rk.merge(
            unmatched_sp2d,
            left_on=['jumlah', 'tanggal', 'skpd_keterangan'],  # Kunci dari RK
            right_on=['jumlah', 'tglsp2d', 'skpd_code'],       # Kunci dari SP2D
            how='inner',
            suffixes=('', '_y')
        )
        ...
        if not second_merge.empty:
            ...
            used_sp2d.update(second_merge['nosp2d_y'])
            unmatched_sp2d = sp2d_df[~sp2d_df['nosp2d'].isin(used_sp2d)]
