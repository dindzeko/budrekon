...
def extract_sp2d_number(description):
    """
    Fungsi untuk mengekstrak nomor SP2D dari uraian.
    - Jika ada lebih dari 6 digit, ambil 6 digit terakhir.
    - Jika kurang dari 6 digit, tambahkan angka 0 di depan hingga menjadi 6 digit.
    """
    match = re.search(r'\d{1,}', str(description))  # Cari semua angka dalam uraian
    if match:
        sp2d_number = match.group(0)[-6:]  # Ambil 6 digit terakhir
        return sp2d_number.zfill(6)  # Pastikan panjangnya 6 digit dengan menambahkan 0 di depan jika perlu
    return None
